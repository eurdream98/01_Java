...
public class Application01 {
    public static void main(String[] args) {
        ...
        * 레퍼런스 타입은 List타입으로 해 두는 것이 더 유연한 코드를 작성하는 방법이다. */
        ...
        * toString()메소드가 오버라이딩 되어있어 담겨있는 요소들을 확인할 수 있다. */
        ...
        * 내부적으로 관리되는 배열의 사이즈는 외부에서 알 필요가 없으므로 기능으로 제공하지 않는다.*/
        ...
        /*인덱스에 해당하는 값을 가져올 때는 get메소드를 사용한다.*/
        for(int i = 0 ; i<alist.size();i++){
            System.out.println(i+":"+alist.get(i));
        }

        /*데이터의 중복 저장을 허용한다.
        * 배열과 같이 인덱스로 요소를 관리하기 떄문에 가능하다.*/
        alist.add("apple");
        System.out.println("alist : "+alist);
        ...
        /*저장 된 값을 삭제할 떄 remove() 메소드를 사용한다.
        * 중간 인덱스의 값을 삭제하는 경우 자동으로 인덱스를 하나씩 앞으로 당긴다.
        * */
        alist.remove(2);
        System.out.println("alist:"+alist);

        /*지정 된 위치의 값을 수정할 떄도 인덱스를 활용하여 set()메소드를 사용한다.*/
        alist.set(1,true);
        System.out.println("alist:"+alist);
