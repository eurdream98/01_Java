...
public class FireCar extends Car{
    /*클래스 선언부에 extendCar를 추가해서 Car클래스를 상속 받는다.
    ...
    */
    ...
    /*@Override 어노테이션
    * JDK1.5부터 추가 된 문법으로 오버라이딩 성립 요건을 체크하여 성립 되지 않는 경우 컴파일 에러를 발생시킨다.
    * 오버라이딩 하는 메소드는 기본적으로 부모 메소드 선언 내용을 그대로 작성해야한다.*/
    ...
    public void soundHorn(){
        if(isRunning()){
            ...
            * Car클래스의 runningStatus를 protected로 변경하여 직접 접근할 수도 있지만 캡슐화의 원칙에 위배되므로 권장하지 않는다.
            * this.isRunning() or super.isRunning()둘 다 사용 가능하며 컴파일러는 기본적으로 this.를 추가한다.
            * (상황에 따라서는 super. 를 명시적으로 써야하는 상황도 있다.)*/
            System.out.println("빠아아아아아아아앙~!!빠아아아아아아앙앙~!!");
        }else{
