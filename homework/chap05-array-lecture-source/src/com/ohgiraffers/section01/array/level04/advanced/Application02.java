...
public class Application02 {
    public static void main(String[] args) {
        ...
        int strike = 0;
        while(cnt>0){
            System.out.print(cnt+"회 남으셨습니다.");
            System.out.print("4자리 숫자를 입력하세요 : ");
            int choice = sc.nextInt();
            if(choice<1000){
                System.out.println("네 자리 수를 입력해주세요.");
                continue;
            }
            int[] choiceArr = new int[answer.length];
            int num = 1000;
            for(int i = 0 ; i<choiceArr.length;i++){
                choiceArr[i] = choice/num;
                choice %= num;
                num /= 10;
            }
            ...
            for(int i = 0 ; i<choiceArr.length;i++){
                for(int j = 0 ; j<choiceArr.length;j++){
                    if(answer[i]==choiceArr[j]&&i==j){
                        strike++;
                    }else if(answer[i]==choiceArr[j]&&i!=j){
                        ball++;
                    }
                }
            }
            choiceArr = null;
            System.out.println("아쉽네요"+ball+"B "+strike+"S 입니다.");
            cnt--;
            if(cnt==0){
